    ...
    try{
        ...
        if(post.coverImgSrc){
            ...
            if(coverImgFile){
                ...
                fs.unlinkSync(path.join(__rootDir,'../../', prevCoverImgSrc))
                // 새로운커버추가
                const fileName = Date.now() + '-' + coverImgFile.originalname
                const nextCoverImgSrc = `/public/postImgs/${post._id}/cover/${fileName}`
                fs.writeFileSync(path.join(__rootDir,'../../', nextCoverImgSrc), coverImgFile.buffer, 'binary')
        ...
        for(let key in nextEntityMap){
            if(nextEntityMap[key].type === "IMAGE"){
                ...
                if(middleFolderName === 'public'){
                    ...
                    const oldPath = path.join(__rootDir,'../../', entityImgSrc)
        ...
        prevImgSrcs.forEach((prevImgSrc) => {
            if(nextImgSrcs.indexOf(prevImgSrc) === -1){
                //해당이미지 삭제
                const imgPath = path.join(__rootDir,'../../', prevImgSrc)
