...
const createPost = async (req, res) => {
    try{
        ...
        fs.mkdirSync(path.join(__rootDir, '../../', `/public/postImgs/${post._id}`))
        fs.mkdirSync(path.join(__rootDir, '../../', `/public/postImgs/${post._id}/cover`))
        ...
        const coverImgSrc = !coverImgFile 
            ...
            : (function() {
                ...
                fs.writeFileSync(path.join(__rootDir, '../../', coverImgSrc), coverImgFile.buffer, "binary")
                return coverImgSrc
            })()
        
        // contentState ( entitymap img src update ( temp => rea ) )
        const tempImgFileNames = fs.readdirSync(path.join(__rootDir, '../../','/public/temporary/postImgs'))
        ...
        for(let key in entityMap){
            if(entityMap[key].type === 'IMAGE'){
                ...
                const oldPath = path.join(__rootDir, '../../' ,tempImgSrc)
                const newPath = path.join(__rootDir, '../../' ,imgSrc)
        ...
        const remainingTempImgFiles = fs.readdirSync(path.join(__rootDir, '../../' ,'/public/temporary/postImgs'))
        remainingTempImgFiles.forEach((tempImgFileName) => {
            fs.unlinkSync(path.join(__rootDir, '../../' ,'/public/temporary/postImgs', tempImgFileName))
        })
